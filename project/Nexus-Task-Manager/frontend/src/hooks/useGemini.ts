...
import { computeWorkload, computeStreak, computeProductivityScore } from './useProductivity';

const GEMINI_KEY_LS = 'nexus_gemini_key';
const GEMINI_MODEL_LS = 'nexus_gemini_model';
...
export function getGeminiModel(): string {
  return localStorage.getItem(GEMINI_MODEL_LS) || 'gemini-2.0-flash';
}
...
export async function geminiRecommend(tasks: Task[], user: User | null): Promise<Recommendation[]> {
  ...
  const ctx = {
    today,
    user: user ? user.fullName.split(/\s+/)[0] : 'there',
...
export function heuristicRecommend(tasks: Task[]): Recommendation[] {
  const today = iso(new Date());
